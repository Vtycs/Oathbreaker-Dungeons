...
Oathbreaker Dungeons is a 2D action roguelite featuring procedural room generation, a buff/rune and archetype system, and **online co-op via Steam** for up to 4 players. The project was independently developed — from the engine to platform deployment — and is available on Steam.
...
**Protocol features:**
  ...
  - Hardened reliability layer: fixed Buffer/contextIsolation issues, corrected reliable-send logic, resolved packet parsing bugs, and synchronized room seed generation across clients
...
### Rendering Scale & High Refresh-Rate Support

Following a move to a 2K (2560×1440) 144Hz display, the renderer gained a supersampling render scale (`RENDER_SCALE`) decoupled from game logic: the simulation still runs on a fixed logical `frameCount` at 60 ticks/second, while rendering unlocks to the display's native refresh rate.
...
### Buff, rune and archetype system

The game features over 60 unique buffs, 8 archetypes, and an expanded rune combo system with 113 combinations (**Caos Verdadeiro**), plus a persistent account level system for meta-progression. The `addBuff()` method coordinates all chain effects:
...
### Weapon Fusion — FORMAS System

Weapon fusion originally stacked simple stat buffs on top of each other — "a buff on top of a buff, with no real identity." That design was scrapped in favor of distinct mechanical and visual identities per combination: each fusion becomes its own weapon archetype (a FORMA), not just a numbers upgrade.

Examples:

  - Pistol + Pistol → dual pistols alternating shots
  - Shotgun + Spear → a shotgun that fires piercing spears

Implemented FORMAS include: **Akimbo**, **Burst**, **Live Ammo**, **Bayonet**, **Double Barrel**, **Colossus**, and **Twin Blades**.
...
## Solved challenges
  ...
  - **Networking reliability hardening** — fixed Buffer/contextIsolation bugs, corrected reliable-send logic, resolved packet parsing edge cases, and synchronized room seed generation across clients
  - **High refresh-rate rendering decoupled from simulation** — supersampling render scale (`RENDER_SCALE`) while game logic keeps ticking at a fixed 60/s via logical `frameCount`
...
*Developed by Empty TEAM -Poggerz*# Oathbreaker Dungeons V2.0.2 :)
